#!/bin/bash
# Script to run AlgoEnvisioner in a docker container

docker_run_AlgoEnvisioner() {
	...
	then
		docker build -t algoenvisioner .
	fi

	# Run algovision image forwarding internal docker port 5000 to local port 5000
	docker run -p 5000:5000 algoenvisioner python /app/run.py
	return $?
}
